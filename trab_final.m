...
for i = 1:length(Eb_N0_lin)
    ...
    NCod = NA(i)*complex(randn(length(qpsk_Mod_Cod), 1), randn(length(qpsk_Mod_Cod), 1))*sqrt(0.5)*3/4;
    rSemCod = qpsk_Mod + NSemCod; % vetor recebido
    rCod = qpsk_Mod_Cod + NCod;
    mensagemDemod = pskdemod(rSemCod, mod_QPSK, pi/4);
    mensagemDemodDecodHard = ldpcDecoderHard.step(pskdemod(rCod, mod_QPSK, pi/4));
    mensagemDemodDecodSoft = ldpcDecoderSoft.step(pskdemod(rCod, mod_QPSK, pi/4));
...
for i = 1:length(Eb_N0_lin)
    ...
    NCod = NA(i)*complex(randn(length(qamModCod), 1), randn(length(qamModCod), 1))*sqrt(0.5)*3/4;
    rSemCod = qamMod + NSemCod; % vetor recebido
    rCod = qamModCod + NCod;
    mensagemDemod = qamdemod(rSemCod, mod_QAM, 0 ,'Gray');
    mensagemDemodDecodHard = ldpcDecoderHard.step(qamdemod(rCod, mod_QAM, 0 ,'Gray'));
    mensagemDemodDecodSoft = ldpcDecoderSoft.step(qamdemod(rCod, mod_QAM, 0 ,'Gray'));
